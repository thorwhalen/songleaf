...
| 3 | how a song becomes a sheet: `renderer=` on `sheet` | `render_dense_a4`: one A4 page, lyric font size maximised to fit, lines packed within a section, chords overlapping the top of the lyric line in a lighter shade (`reportlab`) | the layouts of songleaf#4 (inline weave, two-column); audio/video renderers on the av side (`reelee`, `muvid`) |

```
Surface for v1: CLI only (MCP/skill/HTTP/frontend: questions answered, not built)
NOT seams: fuzzy scorer (rapidfuzz WRatio), chords-over-lyrics parsing rules, style knobs (colours, chord scale, margins, page size: keyword args), store key encoding, CLI output format
```
